...
def _format_datetime_range(start_iso: str, end_iso: str) -> str:
    """Format a start/end ISO pair into a readable date range string."""
    try:
...
def format_backtest_progress(
    processed: int, total: int, signals: int, wins: int
) -> str:
    """Format a backtest progress update message."""
    pct = processed / total * 100 if total else 0
    wr = wins / signals * 100 if signals else 0
    bar_len = 16
    ...
    return (
        f"\u23f3  <b>Backtesting...</b>\n\n"
        f"<code>{bar}  {pct:.0f}%\n"
        f"Candles   {processed:,} / {total:,}\n"
        f"Signals   {signals}   ({wins}W  {wr:.0f}%)</code>"
    )


def format_backtest_result(result) -> str:
    """Format completed backtest results as an HTML Telegram message.

    Args:
        result: BacktestResult dataclass instance.

    Returns:
        HTML-formatted message string.
    """
    if result.error:
        safe = _escape_html(str(result.error)[:300])
        return f"\u274c  <b>Backtest failed</b>\n\n<code>{safe}</code>"

    if result.total_signals == 0:
        return (
            "\U0001f4ca  <b>BACKTEST COMPLETE</b>\n\n"
            f"Scanned <b>{result.actual_candles:,}</b> candles — "
            "no signals generated.\n"
            "Try a larger window or lower the confidence threshold."
        )

    # Time range
    time_range = _format_datetime_range(result.start_time, result.end_time)

    # PnL
    pnl_sign = "+" if result.total_pnl >= 0 else ""
    avg_sign = "+" if result.avg_pnl >= 0 else ""
    ...
    lines.extend([
        ...
        "",
        "\U0001f3c6 <b>Results</b>",
        "<code>"
        f"Signals          {result.total_signals}\n"
        f"Record           {result.wins}W - {result.losses}L\n"
        f"Win Rate         {result.win_rate:.1%}\n"
        f"P&amp;L              {pnl_sign}${abs(result.total_pnl):.2f}\n"
    ...
    if result.strong_signals or result.normal_signals:
        lines.append("\u26a1 <b>By Confidence</b>")
        lines.append("<code>")
        if result.strong_signals:
            lines.append(
                f"STRONG           {result.strong_wins}W / {result.strong_signals}  "
                f"({result.strong_win_rate:.1%})"
            )
        if result.normal_signals:
            lines.append(
                f"NORMAL           {result.normal_wins}W / {result.normal_signals}  "
                f"({result.normal_win_rate:.1%})"
            )
        lines.append("</code>")
        lines.append("")

    # Direction breakdown
    if result.up_signals or result.down_signals:
        lines.append("\U0001f7e2\U0001f534 <b>By Direction</b>")
        lines.append("<code>")
        if result.up_signals:
            lines.append(
                f"UP               {result.up_wins}W / {result.up_signals}  "
                f"({result.up_win_rate:.1%})"
            )
        if result.down_signals:
            lines.append(
                f"DOWN             {result.down_wins}W / {result.down_signals}  "
                f"({result.down_win_rate:.1%})"
            )
        lines.append("</code>")
        lines.append("")

    # Streaks
    lines.append("\U0001f525 <b>Streaks</b>")
    lines.append(
        f"<code>Best Win         {result.longest_win_streak}\n"
        f"Worst Loss       {result.longest_loss_streak}</code>"
    )
    lines.append("")
    ...
    if result.model_accuracy is not None:
        lines.append(
            f"<code>Model Accuracy   {result.model_accuracy:.1%}</code>"
        )

    # Skipped
    if result.skipped_low_confidence:
        lines.append(
            f"<code>Skipped          {result.skipped_low_confidence} low-confidence</code>"
        )
